...
parser.add_argument('--learning_rate', type=float, default=0.003, help='Initial learning rate [default: 0.001]')
...
#     ds = slices.shuffle(len(labels)).batch(MAX_NUM_POINT, drop_remainder = True) 
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(GPU_INDEX)):
            ...
            tf.summary.scalar('bn_decay', bn_decay)  # Writes simple numeric values for later analysis in TensorBoard. 
            ...
            #print("result")
            #print(pointclouds_pl)  # 32, 1024, 3
            #print(pred)  # 32, 1024, 50
            #print(labels_pl) # 32, 1024
            #print(tf.argmax(pred, 2))  # 32, 1024
        ...
        train_writer = tf.summary.FileWriter(os.path.join(LOG_DIR, 'train'),  # provides a mechanism to create an event file in a given directory and add summaries and events to it
                                             sess.graph)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    if True:
        ...
        for batch_idx in range(num_batches):
            ...
            # for i in range(start_idx, end_idx):
            #     l = current_label[i]
            #     total_seen_class[l] = total_seen_class[l]+ 1
            #     total_correct_class[l] += (pred_val[i - start_idx] == l)
    
    total_seen_class[0] =  total_seen_class[0]+1    
    ...
    log_string('eval avg class acc: %f' % (
        np.mean(np.array(total_correct_class[1:]) / np.array(total_seen_class[1:], dtype=np.float))))
